...
import { Zap, Target, Megaphone, DollarSign, Users, Phone } from "lucide-react"
...
いいか、車選びドットコム等に金払って問い合わせ取るのも大事だけどな、
...
- ��カウント作ったなら → **プロフィールを完成させろ** 。写真・住所・電話番号全部入れろ
...
export function ConsultantChat() {
  return (
    <SharedChatLayout
      ...
      suggestedLabel="今のお悩みは？"
      generateResponse={(question, messageCount) => generateConsultantResponse(question, messageCount ?? 0)}
  theme="consultant"
  inputPlaceholder="悩みを吐き出せ..."
  typingDelay={1800}
  profileInfo={{
    name: "鬼塚 剛",
    title: "辛口経営コンサルタント",
    avatarSrc: "/images/consultant-harsh-cartoon.jpg",
    description: "歯に衣着せぬ辛口アドバイスが信条。中古車販売業界で20年以上の経験を持ち、数々の経営危機を乗り越えてきた実績から、甘い言葉ではなく本当に必要な指摘を行います。",
    specialties: ["経営戦略", "収益改善", "組織改革", "コスト削減", "事業再生", "SNS集客"],
    background: "大手自動車ディーラーで店長・エリアマネージャーを歴任後、独立。中小企業の経営改善コンサルタントとして200社以上を支援。厳しいが的確なアドバイスで、クライアントの業績をV字回復させた実績多数。年商50億を達成した経験を持つ。",
  }}
  />
  )
}
