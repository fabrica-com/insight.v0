        ...
        <DialogContent className="sm:max-w-lg">
          ...
          {purchased ? (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-amber-700 leading-relaxed">
                      ...
                      {selectedCredit?.toLocaleString()}円 /{" "}
                      {((CREDIT_OPTIONS.find((c) => c.amount === selectedCredit)?.tokens ?? 0) / 1000).toFixed(0)}
                      Kトークン）を購入します。この料金は今月の月額利用料と合わせてご請求されます。
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 py-2">
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={cn(
                          ...
                          selectedCredit === option.amount ? "bg-primary text-primary-foreground" : "bg-muted",
                        )}
                      >
              ...
              <DialogFooter>
                <Button className="w-full gap-2" disabled={selectedCredit === null} onClick={handlePurchase}>
                  <Sparkles className="h-4 w-4" />
                  {selectedCredit ? `${selectedCredit.toLocaleString()}円のクーポンを購入` : "プランを選択してください"}
                </Button>
              </DialogFooter>
            </div>
          )}
