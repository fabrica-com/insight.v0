...
const suggestedTopics = [
  ...
  { icon: Camera, text: "カーセンサーの写真を改善したい" },
...
すでに登録済みなら、次のステップをお伝えします。`
    ...
    if (q.includes("写真") || q.includes("カーセンサー") || q.includes("掲載") || q.includes("グーネット")) {
...
【重要なポイント】
小規模店がカーセンサーとグーネット両方に出すのはおすすめしません。
ユーザー層が大きく重複しており、費用が倍になるだけです。
「両方に2万円ずつ」より「カーセンサーに4万円で質を上げる」方が反響が出やすいです。
...
2. 今使っている広告媒体は何ですか？（カーセンサー、グーネット等）
